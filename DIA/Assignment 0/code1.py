...
img = np.array(img)
dic = make_dic(img)
#lis = pop_sort(dic,2000)

# print("Start cut")
lis1 = completeMedianCut(dic.keys(),100)
lis = cal_avg(lis1)
# print("ho gaya")
...
for i in range(len(img)):
	...
	for j in range(len(img[0])):
		...
		new_img[i][j] = tuple(map(normalize,cl_pt))
		err = tuple(map(lambda x : x[0]-x[1], zip(point,cl_pt)))
		if (j+1)<len(img[0]):
		...
		# err = [0,0,0]
		# err[0] = point[0] - cl_pt[0]
		# err[1] = point[1] - cl_pt[1]
		# err[2] = point[2] - cl_pt[2]
		# img[i][j+1][0] += err[0]*(3/8)  
